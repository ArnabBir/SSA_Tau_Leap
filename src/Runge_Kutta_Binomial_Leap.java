 ...
 */
public class Runge_Kutta_Binomial_Leap {
    ...
    public static void main(String[] args) throws IOException {
        ...
        FileWriter writer = new FileWriter("Runge_Kutta_Binomial_Leap.csv");
...
//            }
            ...
            double [][] hf1 = new double[REACTANTS][REACTIONS];
            double [][] hf2 = new double[REACTANTS][REACTIONS];
            double [][] hf3 = new double[REACTANTS][REACTIONS];
            double [][] hf4 = new double[REACTANTS][REACTIONS];

            for (int i = 0; i < REACTANTS; ++i) {
                for (int j = 0; j < REACTIONS; ++j) {
                    if (SMatrix[j][i] == -1) {
                        hf1[i][j] = a[j];
                        hf2[i][j] = a[j] / x[i] * (x[i] + tao / 2 * hf1[i][j]);
                        hf3[i][j] = a[j] / x[i] * (x[i] + tao / 2 * hf2[i][j]);
                        hf4[i][j] = a[j] / x[i] * (x[i] + tao * hf3[i][j]);
                    }
                }
            }
            ...
            for( int i = 0; i < REACTANTS; ++i){
                ...
                for (int j = 0; j < REACTIONS; ++j)
                {
                    mean_value +=  1.00 / 6.00 * (hf1[i][j] + 2 * hf2[i][j] + 2 * hf3[i][j] + hf4[i][j]) * SMatrix[j][i];
                }
                x[i] += (int) Math.round(mean_value * tao);
            }
