...
public class Trapezoidal_Implicit_K_Leap {
        ...
        public static int K_Generator(int[] x, int[][] SMatrix, int[] itr, boolean[] is_critical, boolean bool) {
            ...
            for (int i = 0; i < REACTANTS; ++i) {
                ...
                for (int j = 0; j < REACTIONS; ++j) {
					...
					/*
					 * epsilon_x = EPSILON / itr[j] * x[i]; System.out.println(
					 * "epsilon_x = " + String.valueOf(epsilon_x)); if
					 * (epsilon_x > 1) { //System.out.println("Greater than 1");
					 * max_x_array[i] = epsilon_x; } else { max_x_array[i] = 1;
					 * }
					 */
                ...
                // System.out.println("max_x_array = " +
                // String.valueOf(max_x_array[i]));
                // System.out.println("max_SMatrix = " +
                // String.valueOf(max_SMatrix[i]));
        ...
        public static boolean[][] Affects(int[][] SMatrix) {
            boolean[][] Affects_Matrix = new boolean[REACTIONS][REACTANTS];
            for (int i = 0; i < REACTIONS; ++i) {
                for (int j = 0; j < REACTANTS; ++j) {
                    if (SMatrix[i][j] != 0) {
                        Affects_Matrix[i][j] = true;
                    } else {
                        Affects_Matrix[i][j] = false;
                    }
                }
            }

            return Affects_Matrix;

        }

        public static boolean[][] Depends_on(int[][] SMatrix) {
            boolean[][] Depends_on_Matrix = new boolean[REACTIONS][REACTANTS];
            for (int i = 0; i < REACTIONS; ++i) {
                for (int j = 0; j < REACTANTS; ++j) {
                    if (SMatrix[i][j] < 0) {
                        Depends_on_Matrix[i][j] = true;
                    } else {
                        Depends_on_Matrix[i][j] = false;
                    }
                }
            }

            return Depends_on_Matrix;

        }

        public static boolean[][] Dependency_Graph(int[][] SMatrix) {

            boolean[][] Affects_Matrix = Affects(SMatrix);
            boolean[][] Depends_on_Matrix = Depends_on(SMatrix);
            boolean[][] Dependency = new boolean[REACTIONS][REACTANTS];

            for (int i = 0; i < REACTIONS; ++i) {
                for (int j = 0; j < REACTIONS; ++j) {
                    for (int k = 0; k < REACTANTS; ++k) {
                        if (Affects_Matrix[i][k] == true && Depends_on_Matrix[j][k] == true)
                            Dependency[i][j] = true;
                    }
                }
            }
        ...
        public static void main(String[] args) throws IOException {
            ...
            //FileWriter writer = new FileWriter("Trapezoidal_Implicit_KLeap_Output.csv");
...

                {
                    ...
                    if (tao_dash < tao_2dash) {
                        ...
                        if (not_critical < REACTIONS) {
                            ...
                            // System.out.println(a0_c + " " + tao + " " + Kc + " "
                            // + Knc);
                    ...
                    double[] a_new = new double[REACTIONS];
                    ...
                    for (int i = 0; i < REACTANTS; ++i) {
                        for (int j = 0; j < REACTIONS; ++j) {
                            // System.out.println("K = " + K[j]);
                            //System.out.println("K = " +  K[j] +  "a[] = "+ (int)(a_new[j] * tao / 2 - a[j] * tao / 2));
                            x[i] += (int) (Math.round(a_new[j] * tao / 2 - a[j] * tao / 2) ) * SMatrix[j][i];
                        }
                    }
