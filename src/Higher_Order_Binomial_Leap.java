...
public class Higher_Order_Binomial_Leap {
    ...
    public static void main(String[] args) throws IOException {
        ...
        FileWriter writer = new FileWriter("Higher_Order_Binomial_Leap.csv");
...
//                x[i] = xp[i];
                for (int j = 0; j < REACTIONS; ++j)
                {
                    x[i] += Math.round( ap[j] * tao / 2 + Bp[j] - a[j] * tao / 2) * SMatrix[j][i];
                }
            }
...
//            }
                ...
                for (int i = 0; i < REACTANTS; ++i)
                {
                    ...
                    for (int j = 0; j < REACTIONS; ++j)
                    {
                        x[i] += Math.round( ap[j] * tao / 2 + Bp[j] - l[j] * tao / 2) * SMatrix[j][i];
                    }
                }
