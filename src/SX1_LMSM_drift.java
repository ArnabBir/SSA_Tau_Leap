...
//        String xyz = sc.nextLine();
            ...
            FileWriter writer = new FileWriter("SC1+drift_2RK_4ABMMSM_PC_Binomial_Leap.csv");
...
//                    }
                    ...
                    for (int i = 0; i < REACTANTS; ++i) {
                        xp[iteration][i] = x[i];
                        for (int j = 0; j < REACTIONS; ++j) {
                            xp[iteration][i] += SMatrix[j][i] * 2.00 * a[j] * tao / 3.00 ;
                        }
                    }
...
//                    x[i] = xp[iteration][i];
                    ...
                    for (int j = 0; j < REACTIONS; ++j) {
                        ...
                        double mean_equivalent = 4.00 / 3.00 * (2.00 * ap[3][j] * tao - ap[2][j] * tao + 2.00 * ap[1][j] * tao);
                    ...
                    double [] mean_equivalent = new double [REACTIONS];
                    for (int j = 0; j < REACTIONS; ++j) {
                        //System.out.println("product[j] = " + x_predicted[j] * tao);
                        mean_equivalent[j] = 1.00 / 3.00 * (a_predicted[j] * tao + 4.00 * ap[3][j] * tao + ap[2][j] * tao);
                        //System.out.println("mean_equivalent = " + mean_equivalent);
                        //double p = mean_equivalent / L[j];
                        //System.out.println("p = " + p);
                       // B[j] = Binomial_Random_Generator(L[j], p);
                    ...
                    for (int i = 0; i < REACTANTS; ++i) {
                        x[i] = xp[2][i];
                        for (int j = 0; j < REACTIONS; ++j)
                            x[i] += SMatrix[j][i] * mean_equivalent[j];
