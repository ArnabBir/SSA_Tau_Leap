...
//        String xyz = sc.nextLine();
        FileReader fr = new FileReader("B_Cell_Matrix.csv");
        ...
        int[][] xp = new int[3][REACTANTS];

        double[][] ap = new double[3][REACTIONS];
        ...
        FileWriter writer = new FileWriter("BCell_Multistep_Binomial_Leap.csv");
        ...
        for (int i = 0; i < REACTIONS && ((line = bb.readLine()) != null) ; ++i) {
            //System.out.println("Enter the value of k" + (i + 1) + " :\n");
            k[i] =  Double.parseDouble(line);
...
//                }
            ...
            for (int j = 0; j < REACTIONS; ++j) {
                ...
                //System.out.println("reaction_arrray = " + reaction_array[j] );
...
//            }
            ...
            if (iteration < 3) {
                ...
                for (int j = 0; j < REACTIONS; ++j) {
                    ...
                    double mean_equivalent = (23.00 / 12.00 * ap[2][j] * tao - 16.00 / 12.00 * ap[1][j] * tao + 5.00 / 12.00 * ap[0][j] * tao);
                ...
                for (int j = 0; j < REACTIONS; ++j) {
                    //System.out.println("product[j] = " + x_predicted[j] * tao);
                    double mean_equivalent = (9.00 / 24.00 * a_predicted[j] * tao + 19.00 / 24.00 * ap[2][j] * tao - 5.00 / 24.00 * ap[1][j] * tao + 1.00 / 24.00 * ap[0][j] * tao);
                    //System.out.println("mean_equivalent = " + mean_equivalent);
                    double p = mean_equivalent / L[j];
                    //System.out.println("p = " + p);
                    B[j] = Binomial_Random_Generator(L[j], p);
